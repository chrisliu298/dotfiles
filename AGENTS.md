...
- After `git push` succeeds, run `dfs` to sync the new commit to all peers (concurrent `git pull --ff-only` + `./dotfiles.sh` on each of `macbookpro16`, `macmini`, `l40s`; self is skipped). Skipping this leaves other machines on the previous revision. From a non-interactive shell (e.g., agent tool invocation), use `zsh -c 'source ~/dotfiles/shell/.functions && dfs'` — no TTY required.
