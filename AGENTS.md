...
MCP servers and Claude plugins are wired the same way, via the `MCP_SERVERS` and `PLUGINS` tables in `dotfiles.sh` — see `agents/extensions/README.md` for the full catalog.
