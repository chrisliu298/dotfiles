...
- Delegation routing: use the `dual-subagent` skill for every delegated read-only task, including research, reading, analysis, diagnosis, checks, review, and audit. Native subagents are only for tasks that require changes to files or external state. Split mixed tasks into read-only and modification stages; never use dual-subagent to make changes or native subagents as read-only substitutes. The lead may handle simple tasks directly.
- Delegation threshold: delegate when independent work can reduce completion time or materially improve the result after accounting for startup, context transfer, and synthesis. Batch related read-only questions about the same artifacts into one dual call; do not launch separate calls for every question or re-review unchanged evidence.
- Concurrent subagents: parallelize independent tasks through the appropriate route; continue useful local work, then wait for all results before synthesis. If dual-subagent is unavailable or incomplete, report the gap and follow its retry rules; do not silently substitute a native or single reviewer.
