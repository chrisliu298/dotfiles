...
**GPT and gpt-pro** run their own harness with their own native web search/fetch — use those directly; the `WebSearch` / `WebFetch` and Jina details above are Claude-Code-only. Either way, what matters is that the fact is grounded in a live source.
