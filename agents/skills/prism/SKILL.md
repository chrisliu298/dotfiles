...
- **Lens quality test:**
  ...
  - Never assign the same lens to two of GPT, Grok Build, GLM, Kimi, DeepSeek, or MiMo.
...
1. **Verdict line** — one line, the first thing the eye hits. Fixed token order, `·`-separated:
   ...
   - `<n>/<total>` counts **perspectives that returned, including self** (= `7N+M+1` per the **Counting Contract**: the default `N=1` run is `/8` = self + 7 dispatched; `prism 2` (no gpt-pro) is `/15`; **add each returned gpt-pro lens** — `prism 2 2` that fully returns is `/17`; a gpt-pro-only `prism 0 4` is `/5`; a **no-subagents** run is `6N+M+1` — `prism no subagents` is `/7`) — *not* dispatched-only, *not* lineages. A gpt-pro lens that failed terminally does **not** count toward `total`; name the missing perspective rather than silently shrinking the denominator. The tally below counts *lineages*; the two denominators intentionally differ. (Self is a perspective here even though it does not count toward *dispatch* shape elsewhere.)
...
**Suggested lenses by task type:** `scaffold --preset review|design|diagnosis|compare|research|decision|writing` pre-fills seven lenses by task type — the authoritative ordered sets live in `templates/lens-catalog.json` (`.presets`); edit a preset there, not here (the per-task-type starting sets are listed in README). Implementation has no preset — compose it by hand.
...
- **No all-same-model dispatch (HARD RULE):** The dispatched parallax peers must equal the resolved parallax counts — `6N` by default, minus any tier excluded by explicit user instruction. Via `prism-launch` the manifest's `counts` derives this and the single `parallax` call emits exactly those peers. Zero relay **and** zero subagent calls together is legal only as the `N=0` gpt-pro-only shape (valid only when `M ≥ 1`). A **no-subagents** run (`Variant: no-subagents`) satisfies this trivially — it is `6N` cross-model peers with zero subagents (maximally cross-model); but if relay then goes down it must **refuse or degrade to gpt-pro-only**, never substitute subagents (see Parallax → relay unavailable). A run with **no surviving external lane** (parallax down/empty **and** `M = 0`) is refused.
