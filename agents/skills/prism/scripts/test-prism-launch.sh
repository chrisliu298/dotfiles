...
jq -n --arg p "$PKT4" '{shared_packet:$p,parallax:[{to:"gpt",name:"a",effort:"medium",lens:"Adversarial",lens_desc:"d1"},{to:"deepseek",name:"b",lens:"Completeness",lens_desc:"d2"},{to:"mimo",name:"c",lens:"Consistency",lens_desc:"d3"}],subagents:[{lens:"Correctness",lens_desc:"d5"}]}' > "$CFG4"
...
jq -n --arg p "$PKT5" '{shared_packet:$p,parallax:[{to:"mimo",name:"a",lens:"L1",lens_desc:"d"},{to:"gpt",name:"b",effort:"medium",lens:"L2",lens_desc:"d"},{to:"kimi",name:"g",lens:"L8",lens_desc:"d"},{to:"glm",name:"f",lens:"L7",lens_desc:"d"},{to:"deepseek",name:"c",lens:"L3",lens_desc:"d"}],subagents:[{lens:"L6",lens_desc:"d"}]}' > "$CFG5"
...
echo "$OUT5" | grep -q 'dispatch shape: subagents=1 gpt=1 glm=1 kimi=1 deepseek=1 mimo=1' && ok "dispatch shape printed in canonical order (not alphabetical)" || bad "dispatch shape canonical order"

echo "== prepare + dry-run: effort vs no-knob parallax tiers =="
PKTG="$TMP/prism-effort.md"; make_packet "$PKTG"
# gpt (effort-bearing) + mimo (no effort) accepted together
CFGG="$TMP/effort-config.json"
jq -n --arg p "$PKTG" '{shared_packet:$p,parallax:[{to:"gpt",name:"gp",lens:"Adversarial",lens_desc:"d1"},{to:"mimo",name:"mm",lens:"Outsider",lens_desc:"d2"}],subagents:[]}' > "$CFGG"
expect_ok "prepare accepts gpt (effort-bearing) + mimo (no effort)" "$LAUNCH" prepare --config "$CFGG"
...
[ "$(jq -r '.parallax[0].effort' "$TMP/prism-effortgc-manifest.json" 2>/dev/null)" = "null" ] && ok "mimo effort null despite authored 'high'" || bad "mimo effort null"

# dry-run: gpt carries --effort xhigh; mimo carries no --effort.
...
C10="$TMP/c10.json"; jq -n --arg p "$PKT" '{shared_packet:$p,parallax:[{to:"gpt",name:"dup",lens:"A",lens_desc:"d"},{to:"deepseek",name:"dup",lens:"B",lens_desc:"d"}],subagents:[]}' > "$C10"
...
Shared-Packet: $PKTD
Prism-Mode: partial
Partial-User-Quote: "just gpt, deepseek, and a claude subagent"
...
[ "$(jq -r '.parallax[1].effort' "$MANE" 2>/dev/null)" = "null" ] && ok "dispatch: no-knob mimo derives no effort" || bad "dispatch: mimo effort null"
...
printf '%s\n' "$SC" | grep -q '^To: gpt' && printf '%s\n' "$SC" | grep -q '^To: mimo' && printf '%s\n' "$SC" | grep -q '^To: glm' && printf '%s\n' "$SC" | grep -q '^To: kimi' && ok "scaffold lists all five parallax tiers" || bad "scaffold tiers"
...
{ [ -f "$SCOUT" ] && [ "$(grep -c '^Type:' "$SCOUT")" = "6" ] && grep -q "^Shared-Packet: $SCOUTPKT" "$SCOUT"; } && ok "scaffold --out file is a complete 6-record dispatch with the real packet" || bad "scaffold --out content"
...
printf '{"id":"prism-dg","expected":3,"succeeded":2,"failed":1,"results":[{"to":"gpt","name":"prism-alpha","status":"done","res":"%s","log":"/tmp/a.log"},{"to":"glm","name":"prism-beta","status":"done","res":"%s","log":"/tmp/b.log"},{"to":"mimo","name":"prism-gamma","status":"error","res":null,"log":"/tmp/c.log"}]}' "$RA" "$RB" > "$DGRES"
...
grep -q 'lineage: glm' "$DGOUT"  && ok "digest tags the glm lineage" || bad "digest glm lineage"
...
jq -n --arg rc "$RC" --arg rd "$RD" --arg rn "$RN" --arg rm "$RM" --arg rp "$RP" \
  '{id:"prism-dh",expected:5,succeeded:5,failed:0,results:[
    ...
    {to:"kimi",name:"prism-percent",status:"done",res:$rp,log:"/tmp/x.log"}
  ]}' > "$DHRES"
...
# a full preset (all 6 tiers) passes the default floor with NO flag at all.
...
{ printf 'Shared-Packet: %s\nPrism-Mode: partial\nPartial-User-Quote: "use only claude and gpt"\n\n' "$ASW"
  printf 'Type: subagent\nLens: A\nLens-Desc: d\n\n'
  printf 'Type: parallax\nTo: gpt\nLens: B\nLens-Desc: d\n\n'; } > "$ASWD"
...
[ "$(jq -r '.shape.partial_user_quote' "$MANASW" 2>/dev/null)" = '"use only claude and gpt"' ] && ok "contract: user quote recorded verbatim (audit trail)" || bad "partial quote recorded"
...
{ printf '%s' "$PWARN" | grep -q 'PARTIAL prism' && printf '%s' "$PWARN" | grep -q 'use only claude and gpt'; } && ok "contract: partial warns loudly with dropped tiers + cited quote" || bad "partial warning"
...
[ "$(printf '%s\n' "$NS_OUT" | grep -c '^Type: parallax$')" -ge 5 ] && ok "scaffold --no-subagents keeps the parallax fan" || bad "scaffold --no-subagents parallax fan"
...
# N=2 floor check passes; manifest records 10 parallax + shape.n=2 (preset is N=1 only → hand-build)
PKN2="$TMP/nsn2.md"; make_packet "$PKN2"; NSN2="$TMP/nsn2.dispatch"
{ printf 'Shared-Packet: %s\nPrism-Mode: partial\nVariant: no-subagents\nPrism-N: 2\nPartial-User-Quote: "no subagents"\n' "$PKN2"
  i=0; for p in gpt glm kimi deepseek mimo; do for c in a b; do i=$((i+1)); printf '\nType: parallax\nTo: %s\nLens: L%s\nLens-Desc: d\n' "$p" "$i"; done; done; } > "$NSN2"
expect_ok "no-subagents N=2 floor check passes (10 parallax)" "$LAUNCH" prepare --dispatch "$NSN2"
[ "$(jq -r '.counts.parallax_total' "$TMP/nsn2-manifest.json" 2>/dev/null)" = "10" ] && ok "no-subagents N=2 manifest parallax_total=10" || bad "N=2 parallax_total"
...
{ printf 'Shared-Packet: %s\nPrism-Mode: partial\nVariant: no-subagents\nPrism-N: 1\nPrism-M: 1\nPartial-User-Quote: "x"\n' "$PKM1"
  for p in gpt glm kimi deepseek mimo; do printf '\nType: parallax\nTo: %s\nLens: LB-%s\nLens-Desc: d\n' "$p" "$p"; done; } > "$NSM1B"
...
jq -n --arg p "$PKC" '{shared_packet:$p,
  parallax:[ ["gpt","glm","kimi","deepseek","mimo"][] | {to:., name:("ns-"+.), lens:("L-"+.), lens_desc:"d"} ],
  subagents:[],
...
jq -n --arg p "$PKCG" '{shared_packet:$p,
  parallax:[ ["gpt","glm","kimi","deepseek","mimo"][] | {to:., name:("ng-"+.), lens:("L-"+.), lens_desc:"d"} ],
