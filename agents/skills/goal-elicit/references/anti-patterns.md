...
| Untrusted oracle             | The acceptance oracle or reference dataset is itself wrong — stale/cached values, a different locale or environment than the system under test, or the same tool being judged — so the criterion measures the oracle, not the work (e.g. a benchmark scored against cached values last written by some other tool). | For any criterion resting on an external oracle or reference data, name what produces it and one provenance guard (independent of the system under test, fresh not cached, env/locale pinned). A wrong oracle sends work in the wrong direction. Distinct from "Gamed criterion" (executor corrupts a good verifier) — here the verifier is born wrong. Pairs with goal-drive's "suspect signal" stop condition. |
...
| Vague-direction rejection    | User gives taste/aesthetic direction ("make it premium", "professional") with no measurable criteria, and the skill just rejects it as unverifiable — discarding the real intent instead of capturing it. | **Translate, don't ban.** Record the vague word as design *direction* (Guidance/Assumptions), derive concrete checks from it into the verification plan (screenshots, spacing/type scale, design-token conformance), and bound the polish (≤N focused visual rounds). Never let the vague word itself become a `done_when`. This is the *constructive* complement to "Verification theater" / "Visual rabbit-hole" — those reject the bad criterion; this supplies the good one. Worked recipe: `question-bank.md` §9. |
...
- [ ] Any criterion resting on an external oracle/reference dataset names its source and one provenance guard (independence, freshness, env/locale).
...
- [ ] (Optional mechanical backstop) The written artifact passes `goal-drive/scripts/lint_goal_artifact.py` — the executable check for this very list (required fields, every `done_when` mapped to evidence, no leftover placeholders, no dangerous-vague phrases). goal-elicit does **not** run it (it never runs code); a maintainer, CI, or goal-drive's load-time preflight does. Phase 5 hands the user the one-line command.
