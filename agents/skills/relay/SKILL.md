...
| **Kimi** | An independent lineage (Moonshot Kimi K3) via the **Kimi-for-Coding subscription plan** (`api.kimi.com/coding/`, auth via `ANTHROPIC_API_KEY`). Use for another cross-vendor perspective. Thinking pinned on via the registry (`CLAUDE_CODE_EFFORT_LEVEL=high` — K3 is thinking-only, and `high` is K3's own default); ignores `--effort`. **Native WebFetch + WebSearch both work** (no Jina fallback needed). ~466K usable context — K3's limit is 1M tokens, but a 2MB message cap binds first on real text (vs a usable 1M for DeepSeek/MiMo/GLM). Text-only (no image input via relay). | `relay call --to kimi --name ...` |
