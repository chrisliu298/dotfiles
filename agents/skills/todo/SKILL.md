...
- **The script is an accelerator, not the mechanism.** The Overview's format is documented above, so an agent whose harness can't run the script can hand-render it or skip it — the skill stays portable across Claude and Codex. Run `<this-skill-dir>/scripts/docmaint self-test` to verify the renderer.
...
- **Don't let the Overview become a second `STATUS.md`.** It stays todo-internal — counts, areas, blockers, recency. Plain-English stakeholder interpretation belongs in a separate status doc.
