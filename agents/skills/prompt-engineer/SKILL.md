...
- **Structure** — can a stranger tell role from policy from guidelines from data from examples at a glance? If you can't tell them apart when reading the prompt, the model can't either. If not, split them into clearly labeled sections (XML tags for Claude; headings or explicit blocks for GPT).
...
- **Claude** → `references/claude-prompting.md` — clarity, roles, XML structure, examples, output format, effort and thinking guidance, safety controls, agentic patterns, and a per-model section (Opus 5.5, Opus 5, Sonnet 5, Fable 5/Mythos 5, earlier models) covering the behaviors that differ enough to change the prompt.
