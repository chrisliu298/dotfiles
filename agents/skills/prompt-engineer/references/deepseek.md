...
DeepSeek V4 is an open-weight Mixture-of-Experts family (V4-Pro ≈ 1.6T total / 49B active, V4-Flash ≈ 284B total / 13B active) with a 1M-token context window and up to 384k output tokens. Both models share the same prompt surface. Pro remains the default for the deepest reasoning; Flash is the cheaper (~3x) agent-post-trained option — DeepSeek reports the `0731` Flash build beating **V4-Pro-Preview** on its published agent suites (Terminal Bench 2.1, DeepSWE, Toolathlon, NL2Repo). That is a Flash-vs-Pro-*Preview* result and does not establish Flash as the better agent than the currently served `deepseek-v4-pro`; a direct paired comparison would be needed to claim that.
