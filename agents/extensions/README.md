...
> DeepSeek, MiMo, and MiniMax are reached **through** Claude Code via the `ds`/`mm`/`mx`/… aliases (see `shell/.functions`), so they inherit the Claude column directly.
...
| [prism][p-pr]           | ✓ | — | on     | local + [pub][p-pr] — Multi-perspective parallel review (Claude-only caller; dispatches parallax to Codex + DeepSeek + MiMo + MiniMax via relay) |
...
| [relay][p-rl]           | ✓ | — | on     | local + [pub][p-rl] — Cross-agent relay from Claude to Codex/DeepSeek/MiMo/MiniMax (Claude-only caller) |
