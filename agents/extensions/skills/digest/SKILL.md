...
2. **Short version** the reader can act on without scrolling:
   - Code / changes → a Markdown table with real `|`-delimited columns, one row per file, with whatever columns fit the change (never a single column with `·`-joined fields).
