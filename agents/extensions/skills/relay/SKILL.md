...
**Plan-key fallback.** A `claude-env` peer may declare an optional `"fallback"` stanza — `{ "key_env": "...", "base"?: "...", "model"?: "..." }` (`base`/`model` default to the primary's). When the fallback key var is set in the env, every relay call first sends a 1-token probe to the primary endpoint; on a non-2xx (e.g. the subscription plan's quota is exhausted) it re-probes the fallback and, only if the fallback answers, runs that one call against the fallback key/endpoint/model — otherwise it stays on primary so a transient blip never fails the call. Kimi swaps key only (`KIMI_PLAN_KEY_CN → KIMI_PLAN_KEY_INT`, same endpoint); a fallback may also override `base`/`model`. The `km` launcher mirrors this at launch via `_plan_live_creds` in `shell/.functions`. Peers with no `fallback`, or no fallback key set, skip the probe entirely — zero added latency.
