...
**A skill for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) that lets it delegate work to other reasoning models — currently [Codex](https://github.com/openai/codex) (GPT-5.5), [DeepSeek](https://www.deepseek.com/) V4-Pro, [Xiaomi MiMo](https://platform.xiaomimimo.com/) V2.5-Pro, and [Moonshot Kimi](https://platform.kimi.ai/) K2.6.**
...
Subagents spawn copies of the same model. Relay calls a different model — different training, different reasoning, different blind spots. A cross-model review catches more. With four peer options (Codex, DeepSeek, MiMo, and Kimi), you can run cross-model checks against entirely different lineages: Anthropic ↔ OpenAI ↔ DeepSeek ↔ MiMo ↔ Kimi. Subagents can also invoke Relay (`relay`), combining same-model parallelism with cross-model depth.
...
sequenceDiagram
    ...
    participant P as Peer (Codex, DeepSeek, MiMo, or Kimi)
...
The `call` subcommand wraps the full round-trip: generates the request file, invokes the peer agent, and prints the response content to stdout. The script picks the peer from `--to` (default `codex`; pass `deepseek`, `mimo`, or `kimi` to route elsewhere).
...
- **Kimi** — export `KIMI_API_KEY` in your shell (e.g., `~/.zshenv.local`). Like DeepSeek and MiMo, Kimi is reached via the `claude` CLI with Moonshot's Anthropic-compatible endpoint (`api.moonshot.ai/anthropic`) — no separate binary install required.
...
| `kimi` | `kimi-k2.6` | No effort knob (thinking on by default — binary on/off, no DeepSeek-style tiers) | Requires `KIMI_API_KEY`; reached via the `claude` CLI with Moonshot's Anthropic-compatible endpoint (`api.moonshot.ai/anthropic`). Runs with `ENABLE_TOOL_SEARCH=false` per Moonshot's Claude Code guidance |
...
```bash
relay call --to kimi --name auth-review <<'BODY'
Review src/auth.py for security issues. Run pytest to verify.
BODY
```

The `--name` flag provides a human-readable slug; the script prepends a timestamp and PID automatically (format: `YYYYMMDD-HHMMSS-PID-{name}`). The `--effort` flag controls Codex reasoning depth (`medium` or `xhigh`); it does not apply to DeepSeek (always `max`), MiMo, or Kimi (no effort knob).
...
When running multiple peers in the same dispatch step (e.g., from Prism's Parallax tier), launch each `relay` call in its own background Bash invocation — Codex, DeepSeek, MiMo, and Kimi run concurrently as independent processes.
...
With four peer options, Prism's Parallax dispatches each tier independently: a configurable number of Codex, DeepSeek, MiMo, and Kimi agents launch concurrently as separate Relay calls. Each Parallax agent receives the same full question and context as every local reviewer — only the lens differs.
...
- **Kimi** is invoked the same way via the `claude` CLI with `--dangerously-skip-permissions` and the Moonshot endpoint envelope (matches the local `_kimi()` wrapper) — use only in trusted repos
...
- **Moonshot Kimi-K2.6** — fourth peer target added 2026-06 (Anthropic-compatible endpoint envelope)
