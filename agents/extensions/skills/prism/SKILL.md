...
- **Run:** write packet `/tmp/prism-<id>.md` → `scaffold` (emits the `Prism-Mode: full` roster contract) → edit lenses → `prepare --dispatch …` (default fail-closed floor check on the contract) → launch **all at once** (1 backgrounded `parallax` + `N` Agent calls + `M` gpt-pro) → **wait for every notification** → `results` / `digest` → synthesize verdict-first. A reduced roster needs an explicit `Prism-Mode: partial` + `Partial-User-Quote:` waiver. See *Execution*.
...
**Dispatch invariant (HARD RULE — mechanically enforced):** every tier whose resolved count is `> 0` MUST be dispatched at exactly that count — no silent drops, substitutions, or merges. The **full 8-tier fan at `N` is the floor**: the orchestrator **never self-initiates a partial or asymmetric group**. A tier drops to `0` or takes a non-`N` count **only** when the user explicitly says so (an exclusion or per-tier count — see Invocation Shorthand → Natural-language modifications); autonomous sizing picks `N` but always across **all 8**. `N=0` (drop all eight standard tiers, gpt-pro-only) is itself a user-stated config and is legal only with `M ≥ 1`. This is **not prose-only**: on the `--dispatch` path `prepare` reads the required `Prism-Mode` contract and runs a default fail-closed floor check (`full` + `Prism-N` ⇒ every standard tier + subagents at N, gpt-pro at `Prism-M`), so a partial roster reaches a manifest **only** through an explicit `Prism-Mode: partial` + a verbatim `Partial-User-Quote` (recorded for audit). A run that drops a family for cost, latency, or a flaky/overloaded peer (e.g. GLM 529s) is the exact defect this blocks — unavailability is a *post-dispatch* `result.json` `status: error` (retry with `parallax --only <peer>`), never a reason to omit the record.
...
`N` (and the optional `M`) set the symmetric baseline; named modifications override specific tiers on top of it (an explicit exclusion overrides the "all eight always included" default; on conflicting clauses the more specific or later one wins). Resolve to a final per-tier table, then dispatch exactly that — every tier with resolved count > 0 MUST be dispatched at that count (the **Counting Contract** Dispatch invariant: no skip, substitute, defer, or orchestrator-chosen partial group; exception: `relay` unavailable → substitute a same-model subagent carrying that tier's lens and warn). You — the orchestrator — own resolving the shorthand and NL into the dispatch records **and into the `Prism-Mode` contract** (`full` + `Prism-N` for the symmetric default; `partial` + `Partial-User-Quote` only when the user explicitly asked for a reduced set). `prepare` reads that contract and **enforces it by default, fail-closed**: a `full` dispatch missing any standard tier aborts naming it, and a `partial` without a cited quote (or a dispatch with no `Prism-Mode` at all) is rejected — so a silently-dropped family can no longer reach a manifest (Pre-Launch Check #5).
...
**Web access is not a dispatch concern — don't verify it.** Every peer can reach the web (WebFetch + WebSearch both work, and the two native gaps — MiMo's WebSearch and GLM's WebFetch — are each covered by a verified fallback: Jina Search for MiMo, GLM's native `web_reader` or Jina Reader for GLM, so every tier effectively has both; neither is load-bearing here; see [[relay]]). Kimi K2.7-Code's native WebFetch and WebSearch both work (verified 2026-06-19 on the coding plan), so no fallback is needed. By default Prism front-loads all **stable** evidence in the shared packet (that's what Reference Materials is for), so agents reason over the provided Context rather than re-searching it — **but every agent also gets the shared `## Grounding external facts` block, which directs it to verify any external / time-sensitive fact against a live web source instead of answering from memory** (Claude Code agents — subagents + the GLM/Kimi/DeepSeek/MiMo `claude-env` peers — use native `WebSearch`/`WebFetch` with a Jina fallback; Codex, Grok, and gpt-pro use their own harness's native web tools) — and when the *whole task* is live research, each agent researches independently (see the **Independent research for live-research tasks** rule under Shared Context). Either way, do **not** spend a dispatch-time step checking what the relay transport supports — it's settled and irrelevant to a well-built run.
...
**Declare gpt-pro lenses in the dispatch file — `prepare` composes the launcher; you never hand-inline.** Add one `Type: gptpro` record per lens (`Lens` + `Lens-Desc`, optional `Posture: deep-reasoning|research-grounded`), plus the reference list to inline. gpt-pro runs in a web tab and **cannot read any local file**, so `prepare` (after it froze the packet) builds a self-contained launcher `<id>-gptpro-<slug>.md` = the committed `templates/launcher-gptpro.tmpl` header (carrying the instruction-inversion guard as line 1 + the lens) **+ the frozen packet verbatim** (which already carries the shared **Grounding** block — injected into the packet for every agent) **+ every reference file's contents** under `### <path>` headers **+ the calibration block** (`templates/shared-gptpro-calibration.md`, gpt-pro-only). Templated, not hand-retyped → no drift, no dropped guard.

```text
Shared-Packet: /tmp/prism-<id>.md
Prism-Mode: full                 # the roster contract is still REQUIRED (see Dispatch via prism-launch);
Prism-N: 1                       # set Prism-M to the gpt-pro count. For a gpt-pro-ONLY run use Prism-N: 0
Prism-M: 1                       # (and Prism-M ≥ 1). The standard-tier records are omitted from this fragment.
...
`prepare` appends the canonical `## Constraints` (from `templates/shared-constraints.md`), `## How to answer` (from `templates/shared-how-to-answer.md`), and `## Grounding external facts` (from `templates/shared-grounding.md`) if you omitted them — idempotent, so a re-run won't double any. (If you want a bespoke version of any, include that `##` section yourself and prepare leaves it untouched; Constraints additionally fails closed if a bespoke block drops the anti-recursion guard, How-to-answer and Grounding carry no such safety load.) The packet is **frozen** once `prepare` runs — do not modify it after, since dispatched agents read it live. `Write` already confirms success, so no read-back is needed.
...
**You write one line-oriented dispatch file with the Write tool** (one record per agent — never a shell heredoc), then run two commands. For the symmetric default, run `~/.claude/skills/prism/scripts/prism-launch scaffold --n <N>` first — it prints a ready-to-fill skeleton (the `Prism-Mode: full` / `Prism-N` / `Prism-M` contract + correct records in canonical model order) so you only replace the `FILL` lens names + descriptions. Add `--preset review|design|diagnosis|compare|research|decision|writing` to also pre-fill eight lenses by task type (N=1) — then just edit them to taste. (Asymmetric runs — per-tier counts, exclusions — have no scaffold; author those records by hand, and set `Prism-Mode: partial` + `Partial-User-Quote:` since a hand-authored reduced roster is exactly what the floor check blocks without an explicit user authorization.) The dispatch format is plain `Key: value` lines in blank-line-separated records: no braces, commas, quoting, or escaping, so a free-text lens description can't break it the way hand-authored JSON can. Authoring the config as raw JSON is no longer the default — writing literal text with the Write tool is what removes the escaping surface.
...
**Roster contract (REQUIRED on the `--dispatch` path).** Three top-level lines declare the shape and make full-roster the enforced default — `scaffold` emits them for you, so the full path is the path of least resistance:

```text
Prism-Mode: full
Prism-N: 1            # every standard tier + the subagents must be present at N (the floor check); Prism-M for gpt-pro
Prism-M: 0
```
...
#    The Prism-Mode contract drives a DEFAULT fail-closed roster floor check:
#    Prism-Mode: full + Prism-N → every standard tier + subagents at N (gpt-pro at
#    Prism-M); a missing/off-count tier aborts naming it. No flag needed. (CLI
#    --expect-n <N> [--expect-m <M>] still works and OVERRIDES the contract's N/M.)
...
**Manual fallback (TRUE last resort — only if the script file is genuinely missing):** A bare-name "command not found" is NOT a trigger — re-invoke by the absolute path above. The manual flow applies **only** when `~/.claude/skills/prism/scripts/prism-launch` does not exist at all (a broken install). It bypasses **every** mechanical guard — the roster-enforcement contract, the floor check, the injection guards, the manifest `.shape` — so it is the one path where a partial roster can slip through unchecked; never reach for it to "move faster" or to dodge a floor-check failure (that failure is the system working — fix the dispatch instead). First try to repair the install (`cd ~/dotfiles && ./dotfiles.sh`). If repair is truly impossible: enforce the full roster yourself (one relay call per standard peer + one Agent call per subagent, no family omitted unless the user explicitly authorized it), render each launcher with `sed -e 's|{{SHARED_PACKET_PATH}}|...|g' -e 's|{{LENS_NAME}}|...|g' -e 's|{{LENS_DESC}}|...|g' templates/launcher-<kind>.tmpl`, then dispatch one `relay call --to <peer> --name prism-<slug>` heredoc per parallax tier (Codex `--effort xhigh`, Grok Build `--effort high`, no `--effort` on the rest; background each, `timeout: 3660000`). This is the degraded pre-`prism-launch` flow; prefer the script.

## Pre-Launch Checks

Run these checks before launching. If any fails, rewrite and re-check. **When dispatching via `prism-launch` (the default path), `prepare` mechanically enforces checks 1 and 2, plus **all** of check 5 — the roster contract (`Prism-Mode`) is required, and a `full` shape is floor-checked fail-closed against the declared `N`/`M`, so a silently-dropped tier aborts the run naming the tier (no longer a "your judgment" gap); `parallax` enforces check 0 (relay availability) before dispatch and refuses a manifest with no `.shape`. You still own checks 3 (redundancy) and 4 (lens quality), and the *authorization* judgment behind a `Prism-Mode: partial` waiver — `prepare` records and surfaces the cited quote but cannot verify the user truly said it, so only declare `partial` when they explicitly did.** The descriptions below remain authoritative for the manual fallback and for understanding what `prism-launch` verifies.
...
5. **Dispatch-shape test (CRITICAL):** First resolve the config — every tier's count defaults to `N`, then apply any natural-language modifications (exclusions → `0`, per-tier counts). Total dispatched agents (Claude subagents + the seven parallax tiers) must equal the resolved per-tier counts; self does not count. Enumerate planned calls by type — `--to codex`, `--to grok-build`, `--to grok-composer`, `--to glm`, `--to kimi`, `--to deepseek`, `--to mimo` each equal to that tier's resolved count, and Agent calls equal to the resolved Claude-subagent count. The default symmetric run is `N` of each: `7N` relay calls + `N` Agent calls. If `M` > 0, also `M` `Type: gptpro` records — `prepare` composes each into a `<id>-gptpro-<slug>.md` launcher (listed in the manifest's `gptpro[]`), and you fire `M` backgrounded `gpt-pro` Bash calls, separate from the parallax fan (not relay, not in `7N`+`N`). Confirm the `M` records and the resolved `Reference:` list inline the right files. If any count mismatches the resolved config, fix before launching. **Intent-match (now mechanically enforced by default, not just your judgment):** the full 8-tier fan at `N` is the floor (Counting Contract). On the `--dispatch` path you declare the shape in the contract and `prepare` enforces it fail-closed: `Prism-Mode: full` + `Prism-N: <N>` (what `scaffold` emits) makes `prepare` abort unless every standard tier + the subagents are at exactly `N` (and gpt-pro at `Prism-M`), naming the offending tier — so a partial roster cannot reach a manifest by accident or by omitting a flag. Drop a tier below `N` (or to `0`) **only** with an explicit user exclusion/override, declared as `Prism-Mode: partial` + a verbatim `Partial-User-Quote:` (the double-confirm; recorded in the manifest `.shape`). Still echo the `8N+M` status line so the arithmetic is visible (`8×2+0=16`, never a bare `9`). (A CLI `prepare --expect-n <N> [--expect-m <M>]` still works and OVERRIDES the contract's N/M — useful on the lenient `--config` escape hatch.) **Zero-standard floor:** the `N=0` gpt-pro-only shape is `Prism-Mode: full` + `Prism-N: 0` + `Prism-M ≥ 1`; `Prism-N: 0` with `Prism-M: 0` is the no-op `prepare` rejects. The most common failure is emitting fewer relay calls than the resolved parallax total — now caught mechanically.
...
1. **Write the packet** `/tmp/prism-<id>.md` — just `## Full Question` + `## Context` (with Reference Materials). No Constraints, How-to-answer, or Grounding; `prepare` injects them. If the answer depends on **live online facts** (not just repo/packet evidence), add a `### Live research` block naming what each agent must look up independently — don't front-load the findings (see the **Independent research for live-research tasks** rule).
...
1. Build one canonical shared packet (Full Question + Context — `prepare` injects the Constraints, How-to-answer, and Grounding). Write it to `/tmp/prism-<unique-id>.md` with the Write tool. Decide whether the task needs **live online research**; if so, don't front-load the findings — add a `### Live research` block per the **Independent research for live-research tasks** rule (Shared Context).
