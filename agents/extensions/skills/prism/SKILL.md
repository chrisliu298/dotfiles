...
**Symmetric baseline — all eight models at `N=1`** (the bottom-rung anchor, not a fire-without-thinking default): self + 1 Claude subagent + 1 each of Codex, Grok Build, Grok Composer, GLM, Kimi, DeepSeek, MiMo = **8 dispatched + self** (9 perspectives). This shape is what an autonomous no-config decision *lands on* for the minimal case and what an explicit-but-partial config *fills omitted dimensions from* — a bare invocation always routes through the decision procedure (see **Choosing N and gpt-pro**) and never emits this by shortcut. Required dispatch: **`N` Agent calls + 1 backgrounded `prism-launch parallax` call that fans out the `7N` relay calls** (manual fallback: `7N` separate Bash relay calls). Self does not count. There is **no reasoning-effort choice** — Codex always runs at `xhigh`, Grok Build always at `high`, and the other peers have no knob. All eight models are always included at the chosen `N`; the only way to deviate per-tier — exclude one tier or give a tier its own count — is an explicit natural-language modification (see Invocation Shorthand). (The one positional deviation is `N=0`, which drops *all* eight standard tiers at once for a gpt-pro-only run — legal only with `M ≥ 1`; see Invocation Shorthand.) **GPT-Pro is the lone exception to "always included": it is a separate opt-in tier with its own count `M`, default `0`, never part of the symmetric `N` — added only via the optional second number (see Invocation Shorthand → GPT-Pro tier).**
...
- `N` — how many of **each** of the eight models (Claude subagents, Codex, Grok Build, Grok Composer, GLM, Kimi, DeepSeek, MiMo). Integer `≥ 0`, default `1`. `N ≥ 1` dispatches `N` of every standard tier (total `8N`). **`N = 0` excludes all eight standard tiers at once** — the terse spelling of "gpt-pro-only" — and is legal **only when `M ≥ 1`** (the gpt-pro lenses + self are the whole panel); `prism 0` / `prism 0 0` (no gpt-pro either) is rejected, since zero dispatched agents is not a Prism run — raise `N` or add a gpt-pro count. To drop *some* tiers, use a natural-language exclusion instead. Total dispatched = `8N + M`; self does not count.
