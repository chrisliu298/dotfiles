...
**Reference materials (REQUIRED):** Before building the packet, identify all reference materials relevant to the question — CLAUDE.md files, READMEs, configs, docs, skill definitions, style guides, or any file an agent would need. Include the **absolute paths** in the Context section so every agent can read them. Agents cannot discover references on their own; if a path is not listed, the agent will not consult it.
