...
- **Plan before implementing**: For complex or multi-step tasks, create and maintain a plan with `update_plan`. Re-plan when assumptions break. For simple tasks (documentation edits, single-file changes, small fixes), skip planning and execute directly.
...
- **Self-improvement**: After meaningful corrections, add a concise rule in AGENTS.md (or project notes) when it prevents a recurring mistake.
...
- **Use Codex-native tools**: Prefer `apply_patch` for focused edits and shell commands for bulk operations. Avoid references to provider-specific tools that Codex does not expose.
...
- **Extensions live in `~/dotfiles/agents/extensions/`**: When creating or updating extensions, always write to `~/dotfiles/agents/extensions/<extension-name>/`. This is the canonical location; running `./dotfiles.sh` syncs them to `~/.claude/skills/` and `~/.codex/skills/` automatically.
