...
- **Claude Code**: `c` (personal account, auto-accept), `cc` (continue), `cr` (resume), `cpu` (/push), `cl`/`cm`/`ch`/`cx`/`cmx` (low/medium/high/xhigh/max effort)
- **Codex**: `x` (default=medium), `xn`/`xl`/`xm`/`xh`/`xx`/`xmx` (none/low/medium/high/xhigh/max reasoning), `xc` (resume --last), `scout-papers-scholar-inbox-all` (sequentially scout five awesome lists, showing final messages only)
- **TraeCode**: `tc` (GPT-5.6-Sol, high reasoning, bypass approvals and sandbox)
